...
def test_public_aep_package_number_pattern_allowed():
    assert not is_blocked_paid_or_private_artifact(
        "standards/AEP-002/complete-package.zip"
    )


def test_public_aep_package_wrong_casing_allowed():
    assert not is_blocked_paid_or_private_artifact(
        "standards/aep-003/complete-package.ZIP"
    )
...
def test_delivery_kit_zip_blocked():
    assert is_blocked_paid_or_private_artifact(
...
def test_rsi_governance_proof_page_can_be_standalone():
    text = '<meta name="goalos-page-type" content="standalone-proof"><html><head><title>x</title></head></html>'
    assert is_standalone_proof_page(
...
def test_safe_image_asset_allowed_even_with_private_word():
    assert not is_blocked_paid_or_private_artifact("assets/private-diagram.png")


def test_all_non_allowlisted_zips_blocked():
    assert is_blocked_paid_or_private_artifact("site/GoalOS_Commercialization_Ready_Master_Pack.zip")
    assert is_blocked_paid_or_private_artifact("site/random-public.zip")
