...
PROOF=DATA/"rsi-capability-sla-reliability-mesh-proof.json"
...
def main():
    ...
    releases="".join(f"<tr><td>v{r['generation']}</td><td>{'released' if r['released'] else 'rejected'}</td><td>{esc(r['lesson'])}</td><td>{r['validation']['value_capture_rate_percent']}%</td><td>{r['validation']['sla_breach_rate_percent']}%</td></tr>" for r in proof["rsi_releases"])
