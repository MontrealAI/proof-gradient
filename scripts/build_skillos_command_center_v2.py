...
CONTROL_PAGES = {"index.html","proofs.html","actions.html","receipts.html","multi-agent.html","architecture.html","runbook.html","404.html","force-refresh.html"}
...
def collect_workflows() -> list[dict[str,Any]]:
    ...
    for path in sorted(list(WORKFLOWS.glob("*.yml"))+list(WORKFLOWS.glob("*.yaml"))):
        ...
        items.append({"name":name,"path":str(path.relative_to(ROOT)),"filename":path.name,"triggers":triggers,"has_manual_run":"workflow_dispatch" in text,"url":f"https://github.com/{os.getenv('GITHUB_REPOSITORY','MontrealAI/proof-gradient')}/actions/workflows/{path.name}","latest_status":"","latest_conclusion":"","latest_event":"","latest_run_at":"","latest_run_url":""})
    return items
...
def layout(title: str, active: str, body: str, generated_at: str) -> str:
    repo=os.getenv("GITHUB_REPOSITORY","MontrealAI/proof-gradient")
    nav=[("Home","index.html"),("Proofs","proofs.html"),("Actions","actions.html"),("Multi-Agent","multi-agent.html"),("Receipts","receipts.html"),("Architecture","architecture.html"),("Run / Regenerate","runbook.html"),("GitHub",f"https://github.com/{repo}")]
...
<h2>Run or regenerate</h2><div class='grid3'><div class='card'><div class='eyebrow'>Run one proof</div><p>Open <a href='actions.html'>Actions</a>, choose a proof workflow, then click <strong>Run workflow</strong>.</p></div><div class='card'><div class='eyebrow'>Refresh the hub</div><p>Run <strong>Proof Gradient Command Center Autopublisher v2</strong>. It rebuilds this site and deploys GitHub Pages directly.</p></div><div class='card'><div class='eyebrow'>Verify freshness</div><p>Open <a href='data/command-center-manifest.json'>the manifest</a> and compare its timestamp to this page.</p></div></div>
"""
    atomic(SITE/"index.html", layout("Proof Gradient Public Command Center v2","Home",index_body,generated_at))
...
def main() -> None:
    ...
    urls=["https://montrealai.github.io/proof-gradient/"]+[f"https://montrealai.github.io/proof-gradient/{x}" for x in ["proofs.html","actions.html","receipts.html","multi-agent.html","architecture.html","runbook.html"]]
    urls += [f"https://montrealai.github.io/proof-gradient/{p['href']}" for p in proofs if p.get("href","").endswith(".html")]
    ...
    atomic(DOCS/"SKILLOS_PUBLIC_COMMAND_CENTER_V2.md", f"# Proof Gradient Public Command Center v2\n\nGenerated: `{generated_at}`\n\nRun `Proof Gradient Command Center Autopublisher v2` with `publish_to_repo=true`, `deploy_pages=true`, and `force_rebuild=true`.\n\n## Manifest\n\n```json\n{json.dumps(manifest, indent=2, sort_keys=True)}\n```\n\n## Boundary\n\n{PUBLIC_BOUNDARY}\n")
    print(json.dumps({"status":"BUILT_SKILLOS_COMMAND_CENTER_V2","generated_at_utc":generated_at,"proofs_indexed":len(proofs),"workflows_indexed":len(workflows),"skills_surfaced":len(skills),"site_latest_signal_utc":latest},indent=2,sort_keys=True))
