...
def main() -> None:
    ap=argparse.ArgumentParser(); ap.add_argument('--base-url', default='https://montrealai.github.io/proof-gradient/'); ap.add_argument('--retries', type=int, default=8); ap.add_argument('--delay', type=int, default=15); args=ap.parse_args()
    ...
    for i in range(args.retries):
        try:
            ...
            ok=(MARKER in root and MARKER in idx and MARKER in manifest and 'Proof Gradient Command Center' in root and 'Proof Gradient Command Center' in idx and not any(p in root for p in FORBIDDEN) and '<h1>Capability Governance Twin' not in root)
            if ok:
