...
def read_json(path: Path, default: Any) -> Any:
    ...
    try:
        ...
        print(f"warning: could not parse {path}: {exc}; rebuilding registry")
        return default
...
def card_html(item: dict[str, Any]) -> str:
    ...
    causal = metrics.get("causal_uplift", metrics.get("causal_uplift_p05", metrics.get("transfer", 0)))
    ...
    return f"""<article class='card'>
      ...
      <div class='mini'>
        ...
        <b>{pct(causal)}</b><span>causal / transfer lift</span>
...
def render_index(registry: dict[str, Any], latest_entry: dict[str, Any]) -> str:
    ...
    cards = "".join(card_html(item) for item in proofs[:30] if isinstance(item, dict))
...
<main class='wrap'>
  <section class='hero'>
    ...
    <div class='metrics'>
      ...
      <div class='metric'><b>+{pct(lm.get('causal_uplift'))}</b><br>latest causal uplift</div>
...
def main() -> None:
    ...
    entry = {
        ...
        "headline": "Causal attribution for SkillOS RSI: paired counterfactual replay, ablations, negative controls, bootstrap confidence, and locked holdout cases.",
        "summary": proof.get("thesis", "Validation-gated RSI caused measurable improvement under counterfactual replay."),
        "metrics": {
