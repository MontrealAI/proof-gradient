...
def validate_html(root: Path, path: Path, errors: list[str]) -> None:
    ...
    if cls == "standalone_proof_page":
        # v14 microsite compatibility: existing proof/readiness microsites may be
        # standalone without the normal marketing shell. New standalone pages should
        # include GOALOS-STANDALONE-PROOF metadata, meta descriptions, QUEBEC.AI
        # identity, and a /proof-gradient/ escape link, but legacy microsites are
        # not failed solely for missing those non-shell enhancements.
