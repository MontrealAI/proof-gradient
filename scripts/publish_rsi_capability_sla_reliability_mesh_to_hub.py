...
TITLE = 'Autonomous RSI Capability SLA Reliability Mesh Proof'
...
def main() -> None:
    ...
    f=proof['final']
    entry={'id':PROOF_ID,'title':TITLE,'href':HTML,'json':f'data/{JSON_NAME}','doc':f'docs/{MD_NAME}','badge':f'badges/{BADGE_NAME}','status':proof['status'],'proved':proof['proved'],'value_capture_rate_percent':f['value_capture_rate_percent'],'sla_breach_rate_percent':f['sla_breach_rate_percent'],'reliability_score':f['reliability_score'],'skills_used_count':len(proof.get('skills_used',[])),'virtual_specialist_agents':proof['agent_system']['virtual_specialist_agents'],'specialist_roles':proof['agent_system']['specialist_roles'],'rsi_release_count':proof['rsi_release_count'],'generated_at_utc':proof['generated_at_utc']}
...
<h2 style="font-size:clamp(30px,4.5vw,58px);line-height:.95;margin:0 0 12px">Autonomous RSI Capability SLA Reliability Mesh Proof</h2>
<p style="color:#b8c8d8;font-size:18px;line-height:1.55">SkillOS proves that verified skills can operate as SLA-bound capability services with beautiful skills-used evidence, verifier coverage, rollback planning, incident replay, and zero SLA breach.</p>
