...
SITE = ROOT / "site"
BADGES = ROOT / "badges"
PROOF = DATA / "rsi_enterprise_capability_foundry_proof.json"
...
def constellation(roles: list[str]) -> str:
    cx, cy = 520, 340
    nodes = []
    for idx, role in enumerate(roles[:256]):
        if idx < 48:
            ring, denom, offset = 128, 48, idx
        elif idx < 128:
            ring, denom, offset = 218, 80, idx - 48
        else:
            ring, denom, offset = 306, 128, idx - 128
        angle = -math.pi / 2 + offset * 2 * math.pi / denom + (0.07 if idx >= 48 else 0)
        x, y = cx + ring * math.cos(angle), cy + ring * math.sin(angle)
        r = 5.2 if idx < 48 else 4.2 if idx < 128 else 3.4
        nodes.append(f'<line x1="{cx}" y1="{cy}" x2="{x:.1f}" y2="{y:.1f}"/><circle cx="{x:.1f}" cy="{y:.1f}" r="{r}"><title>{esc(role)}</title></circle>')
    return f"""<svg class="constellation" viewBox="0 0 1040 680" role="img" aria-label="Large specialist-agent organization">
...
def main() -> None:
    ...
    badge = """<svg xmlns="http://www.w3.org/2000/svg" width="372" height="20" role="img" aria-label="enterprise capability foundry RSI proof: passed">
...
<text x="10" y="14" fill="#dff7ff" font-family="Verdana" font-size="11">enterprise capability foundry RSI</text>
<text x="267" y="14" fill="#ffffff" font-family="Verdana" font-size="11">proof passed</text>
</svg>"""
    (BADGES / "rsi_enterprise_capability_foundry_proof.svg").write_text(badge, encoding="utf-8")
...
<title>SkillOS Autonomous RSI Enterprise Capability Foundry Proof</title>
<style>
...
<body>
<nav><strong>SkillOS Enterprise Capability Foundry</strong><div><a href="index.html">Command Center</a><a href="proofs.html">Proofs</a><a href="multi-agent.html">Multi-Agent</a><a href="../data/rsi_enterprise_capability_foundry_proof.json">JSON</a><a href="../docs/rsi_enterprise_capability_foundry_proof.md">Report</a></div></nav>
<main>
<section class="hero">
  <div>
    <div class="eyebrow">MONTREAL.AI / PROOF GRADIENT</div>
    <h1>Enterprise Capability Foundry.</h1>
    <p>Autonomous proof that a large specialist-agent organization recursively improves the coordination protocol that turns enterprise resources into compounding productive capability.</p>
  </div>
  <div class="card">
...
"""
    (SITE / "rsi-enterprise-capability-foundry-proof.html").write_text(html_text, encoding="utf-8")

    print(json.dumps({
        "status": "VISIBLE_OUTPUTS_WRITTEN",
        "html": "site/rsi-enterprise-capability-foundry-proof.html",
        "badge": "badges/rsi_enterprise_capability_foundry_proof.svg",
        "json": "data/rsi_enterprise_capability_foundry_proof.json",
        "markdown": "docs/rsi_enterprise_capability_foundry_proof.md",
    }, indent=2))
