...
TITLE = 'Autonomous RSI Capability Assurance Case Graph Proof'
...
def main() -> None:
    ...
    f=proof['final']
    entry={'id':PROOF_ID,'title':TITLE,'href':HTML,'json':f'data/{JSON_NAME}','doc':f'docs/{MD_NAME}','badge':f'badges/{BADGE_NAME}','status':proof['status'],'proved':proof['proved'],'value_capture_rate_percent':f['value_capture_rate_percent'],'assurance_gap_rate_percent':f['assurance_gap_rate_percent'],'audit_readiness_score':f['audit_readiness_score'],'skills_used_count':len(proof.get('skills_used',[])),'virtual_specialist_agents':proof['agent_system']['virtual_specialist_agents'],'specialist_roles':proof['agent_system']['specialist_roles'],'rsi_release_count':proof['rsi_release_count'],'generated_at_utc':proof['generated_at_utc']}
...
<h2 style="font-size:clamp(30px,4.5vw,58px);line-height:.95;margin:0 0 12px">Autonomous RSI Capability Assurance Case Graph Proof</h2>
<p style="color:#b8c8d8;font-size:18px;line-height:1.55">SkillOS proves that verified skills can become audit-ready assurance cases with beautiful skills-used evidence, claim graphs, controls, verifier independence, and public receipts.</p>
