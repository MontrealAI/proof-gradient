...
CARD = """
<section id="rsi-capital-to-capability-engine-proof" style="margin:48px 0;padding:28px;border:1px solid rgba(255,255,255,.14);border-radius:24px;background:rgba(255,255,255,.06)">
  ...
  <h2 style="font-size:clamp(30px,4vw,54px);line-height:1;margin:0 0 14px">Autonomous RSI Capital-to-Capability Engine</h2>
...
def main() -> None:
    if not INDEX.exists():
        ...
        INDEX.write_text('<!doctype html><html><body><main></main></body></html>', encoding='utf-8')
    text = INDEX.read_text(encoding='utf-8')
    if 'rsi-capital-to-capability-engine-proof.html' in text and 'Autonomous RSI Capital-to-Capability Engine' in text:
        print('RSI capital-to-capability proof card already visible.')
        return
    if '</main>' in text:
    ...
    print('RSI capital-to-capability proof card added to homepage.')
