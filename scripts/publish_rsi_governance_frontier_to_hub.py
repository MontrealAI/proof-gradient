...
def status_rows(proofs: list[dict]) -> str:
    ...
    for p in proofs[:12]:
        ...
        badge_class = "pass" if p.get("proved") else "pending"
        rows.append(f"""<tr><td><a href="{p.get('path','#')}">{p.get('title','Proof')}</a><br><span>{p.get('workflow','')}</span></td><td><b class="{badge_class}">{status}</b></td><td>{p.get('updated_at','')}</td><td>{p.get('capture','')}</td></tr>""")
    return "".join(rows)
...
def render_index(proofs: list[dict]) -> str:
    ...
    title = latest.get("title", "SkillOS Proof Command Center")
...
<section><h2 class="section-title">Latest proof status.</h2><table><thead><tr><th>Workflow</th><th>Status</th><th>Updated</th><th>Capture</th></tr></thead><tbody>{rows}</tbody></table></section>
</main><footer class="wrap">Public benchmark hub. No claim of achieved superintelligence, live revenue, legal advice, policy advice, investment advice, customer results, or Kardashev Type II achievement.</footer>
</body></html>"""
...
def main() -> None:
    ...
    proofs.insert(0, {
        ...
        "short_title": "RSI Governance Frontier Proof",
        "description": "Large autonomous specialist-agent governance lattice with validation-gated RSI, evidence courts, risk courts, locked holdout cases, negative controls, and public receipts.",
        "path": PROOF_PAGE,
