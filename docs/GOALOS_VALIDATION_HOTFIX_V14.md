...
Current recommended path: GoalOS Validation Hotfix v14 Microsite Compatibility; then the actual autonomous deployment workflow that implements GoalOS Public Site Release v8 Intelligent Assets; then Validate GoalOS Public Site v8 only if updated to use v14 shared rules. Validate-only compatibility workflows must not be presented as deployment workflows.
