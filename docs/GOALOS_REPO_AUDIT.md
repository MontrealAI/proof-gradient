...
Required static SVG badges are present under `badges/`, listed in `docs/tables/goalos_badge_inventory.csv`, recorded in `docs/data/goalos_catalog.yml`, and referenced by README. `scripts/validate_goalos_catalog.py` now fails if a required institutional badge file is missing or absent from the catalog badge inventory. No missing required badge was found in the latest validation run.
...
- `python scripts/validate_goalos_catalog.py` — passed; now checks the complete required table, figure, and badge inventory against `docs/data/goalos_catalog.yml`.
