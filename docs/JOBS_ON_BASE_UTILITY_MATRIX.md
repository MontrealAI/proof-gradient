# JOBS on Base Utility Matrix

Genesis utilities: sponsor posting, builder claiming, proof submission bonds, reviewer bonds, credential minting, proof-card registry actions, premium job access, referral attribution, reputation-weighted opportunities.

$JOBS is the native coordination asset for proof-based AI workflow work on Base. $JOBS is additive and does not replace the GoalOS product ladder. $JOBS coordinates proof-based work around GoalOS and must not be promoted as an investment.

Canonical flow: Sponsor posts a job using $JOBS. Builder claims the job with $JOBS. Builder submits proof with a $JOBS bond. Reviewer validates proof with a reviewer bond. Approved proof creates a proof card. Approved proof creates or upgrades a credential. Credential and proof history increase reputation. Reputation unlocks better jobs. Better jobs create more proof.

Technical status: GoalOS $JOBS Base Production Release Candidate v4.2 — Audit-Ready Label-Clean. Audit-ready production release candidate; Base Sepolia first; not audited; not mainnet authorized; not legally approved; not tax reviewed; not guaranteed non-security.

Mainnet gates: npm install; npm run compile; npm test; npm run static-check; deploy to Base Sepolia; run full proof-job loop; external Solidity audit / expert review; legal/token counsel review; tax/accounting review; treasury/founder address verification; operator role review; public claims review; utility live from genesis; mainnet gate approval.

Prohibited claims: investment, profit, yield, revenue share, passive income, price target, guaranteed resale value, guaranteed non-security, audited if not audited, mainnet-ready if not mainnet authorized, founder timelock/no-sale if not actually implemented, legal/tax approval if not completed.
