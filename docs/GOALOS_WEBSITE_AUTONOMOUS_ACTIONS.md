...
Use **GoalOS Public Site Release v8 Intelligent Assets** for public-site refresh. Use **GoalOS Validation Hotfix v14 Microsite Compatibility** as the current validation fix. Do not run v12 or v13. Do not run obsolete v8 compatibility validation as the current path. If site content needs product ladder changes, update `docs/data/goalos_catalog.yml` and relevant action/template sources first, then run the correct GitHub Action and validate after release.
